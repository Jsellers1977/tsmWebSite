...
function createCloud(position) {
  ...
  if (cloud.speed < 40) cloud.opacity = cloud.speed * 0.025;
  else if (cloud.speed > 380) cloud.opacity = (420 - cloud.speed) * 0.025;
  else cloud.opacity = 1;
  clouds.push(cloud);
}
...
function createStars() {
  for (var i = 0; i < 80; i++) {
    ...
    do {
      ...
      (star.x - 300) * (star.x - 300) + (star.y - 225) * (star.y - 225) >
      172.5 * 172.5
    ); /*Check if the coordonates are in the circle*/
